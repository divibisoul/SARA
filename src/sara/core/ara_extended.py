...
class ARA_Extended(ARA):
    ...
    # Preserva a dependência histórica para auditoria sem registrar ciclo real.
    HISTORICAL_DEPENDENCIES = ARA.DEPENDENCIES + ("ARA",)
    ...
    def detect_structural(self, text: str) -> list[StructuralFlaw]:
        ...
        # também são limites reais e evitam falso positivo em saídas já organizadas.
    ...
    def _semantic_replace(self, text: str) -> str:
        """Escolhe sinônimo baseado no contexto e de forma idempotente."""
    ...
    def applied_to_self(self) -> dict:
        """Audita a implementação observável do próprio ARA."""
        import inspect
