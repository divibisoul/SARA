...
from sara.contracts import ModuleRegistry, RegistryError
...
def build_default_system() -> SaraSystem:
    registry = ModuleRegistry()
    registration_report = {"registered": [], "failed": [], "pending": []}
    ...
    loop = RegenerativeLoop(
        ara=ara, etr=etr, itr=itr, identity=identity,
        memory=memory, temporal=temporal, dna=dna,
        filters=filters, rollback=rollback,
        decision_trace=trace, provenance=prov,
        registry=registry, governance_backend=governance_backend,
        cycle_auditor=auditor, max_cycles=3,
    )
    sistema = SistemaVivo(loop, storm, trace, registry=registry, provenance=prov)

    candidates = [
        ara, etr, itr, identity, ubuntu, buen,
        legal_compliance, legal_ai, committee, radar, governed,
        ara_forge, quantum_snapshot, eru, neuro, neural_lens, synergy,
        storm, governance_backend,
        dna, temporal, memory, rollback, filters, safe_sandbox,
        quantum_crawler, quantum_scanner, transystem,
    ]
    for module in candidates:
        name = getattr(module, "NAME", type(module).__name__)
        try:
            registry.register(module)
            registration_report["registered"].append(name)
            if hasattr(module, "STATUS") and module.STATUS.value == "PENDING_INFRASTRUCTURE":
                registration_report["pending"].append(name)
        except RegistryError as exc:
            logger.warning(f"[bootstrap] falha ao registrar {name}: {exc}")
            registration_report["failed"].append({"module": name, "reason": str(exc)})
    ...
    return SaraSystem(
        ...
        components={
            "ara": ara, "etr": etr, "itr": itr,
            "identity": identity, "memory": memory, "temporal": temporal,
            "dna": dna, "filters": filters, "rollback": rollback,
            "trace": trace, "loop": loop, "sistema_vivo": sistema,
            "governed": governed, "radar": radar, "eru": eru,
            "neuro": neuro, "synergy": synergy,
            "ubuntu": ubuntu, "buen": buen,
            "legal": legal_compliance, "legal_ai": legal_ai,
            "committee": committee,
            "storm": storm, "governance": governance_backend,
            "ara_forge": ara_forge, "snapshot": quantum_snapshot,
            "neural_lens": neural_lens,
            "quantum_crawler": quantum_crawler,
            "quantum_scanner": quantum_scanner,
            "safe_sandbox": safe_sandbox,
            "transystem": transystem,
            "auditor": auditor,
            "provenance": prov,
            "activation_plan": CANONICAL_ACTIVATION_PLAN,
        },
    )